...
{
    public class ConfigDiffRecordsService
    {
        private static string sql_connectionString = UniversalService.sql_connectionString;

        // 要導入資料的Table
        // 這邊新增新的Table，排程用的 E:\snap_tool\EA_Postgr2Mysql 這邊也要加
        public static List<string> table_list = new List<string>() {
            "MarketInformation", "PriceStreamProfile", "PriceStreamProfileConfig", "AggregationConfig",
            "VolumeBandConfig", "ExecutionProfiles", "OrderRoutingTables", "VolumeBandConfiguration","SymbolGroups"};

        private static List<string> IgnoreCol_List = 
            new List<string> { "ManualMarketId", "ClearNudgeAtEOD", "ManualMode", "ManualNudge", "NudgeAlertThreshold" };

        // 給前端選的Table
        public static List<string> select_table_list = new List<string>() {
            "Symbol", "Price Stream Profile", "Price Stream Profile Config", "Price Aggregator",
            "Volume Band", "Execution Profile", "Order Routing" };
                ...
                using (var myConn = new MySqlConnection(sql_connectionString))
                {
                    ...
                    foreach (var table_name in table_list)
                    {
                        await Task.Run(async () =>
                        {
                            ...
                            {
                                string deleteSql = $@"
                                        DELETE FROM pe_setting.{table_name}
                                ...
                                using (var pgConn = new NpgsqlConnection(pg_connectionString))
                                {
                                    ...
                                    using (var reader = await cmd.ExecuteReaderAsync())
                                    {
                                        ...
                                        var columnNames = schemaTable.Rows.Cast<DataRow>()
                                            ...
                                            .Where(col => !IgnoreCol_List.Contains(col))
                                            .ToList();
                                        ...
                                        while (await reader.ReadAsync())
                                        {
                                            ...
                                            if (insertValues.Count >= 500)
                                            {
                                                ...
                                                var insertSql = $@"
                                                    INSERT INTO pe_setting.{table_name} ({finalColumnList})
        ...
        public static Dictionary<string, PeServerRecord> GetAllPeServer()
        {
            ...
            using (MySqlConnection connection = new MySqlConnection(sql_connectionString))
            {
                ...
                string query = "SELECT * FROM pe_setting.sql_connect;";
        ...
        public static List<ViewRecord> CompareData_NEW(string _From, string _To)
        {
            ...
            // 忽略欄位
            var ignored_col = new List<string>() { "UpdatedBy", "UpdatedDate", "CreatedBy", "CreatedDate", "INPUT_TIME", "Priority", "AmountDP", "MarketScope" };

            // 使用表格
            var tableMappings = new Dictionary<string, TableRecord>
            {
                ["Symbol"] = new TableRecord
                {
                    primary_key = "MarketId",
                    secondary_key = "Symbol",
                    query = @$"
                            SELECT *
                            FROM pe_setting.MarketInformation
                            WHERE INPUT_TIME IN ('{_From}','{_To}')"
                },
                ["Price Stream Profile"] = new TableRecord
                {
                    primary_key = "Id",
                    secondary_key = "PriceStreamProfileName",
                    query = @$"
                            SELECT Id, PriceStreamProfileName, IsActive, PriceStreamProfileTimeZone, TimeRange, MTSecurityMappingName, INPUT_TIME, REGION
                            FROM pe_setting.PriceStreamProfile
                            WHERE INPUT_TIME IN ('{_From}','{_To}')"
                },
                ["Price Stream Profile Config"] = new TableRecord
                {
                    primary_key = "Id",
                    secondary_key = "Symbol, PriceStreamProfileName",
                    query = @$"
                            SELECT R.PriceStreamProfileName, L.*
                            FROM pe_setting.PriceStreamProfileConfig L
                            LEFT JOIN pe_setting.PriceStreamProfile R 
                                ON L.PricingStreamProfileId = R.Id 
                                AND L.REGION = R.REGION 
                                AND L.INPUT_TIME = R.INPUT_TIME
                            WHERE L.INPUT_TIME IN ('{_From}','{_To}')"
                },
                ["Price Aggregator"] = new TableRecord
                {
                    primary_key = "Id",
                    secondary_key = "Name",
                    query = @$"
                            SELECT *
                            FROM pe_setting.AggregationConfig
                            WHERE INPUT_TIME IN ('{_From}','{_To}')"
                },
                ["Volume Band"] = new TableRecord
                {
                    primary_key = "Id",
                    secondary_key = "VolumeBandName",
                    query = @$"
                            SELECT DISTINCT L.Id, R.VolumeBandName, R.IsActive, L.TimeZone, L.StartTime, L.EndTime, L.VolumebandLayer, L.INPUT_TIME, L.REGION
                            FROM pe_setting.volumebandconfig L
                            LEFT JOIN pe_setting.volumebandconfiguration R 
                                ON L.VolumeBandConfigurationId = R.Id
                                AND L.REGION = R.REGION
                                AND L.INPUT_TIME = R.INPUT_TIME
                            WHERE L.INPUT_TIME IN ('{_From}','{_To}')"
                },
                ["Execution Profile"] = new TableRecord
                {
                    ...
                    query = @$"
                            SELECT *
                            FROM pe_setting.ExecutionProfiles
                            WHERE INPUT_TIME IN ('{_From}','{_To}')"
                },
                ["Order Routing"] = new TableRecord
                {
                    ...
                    query = @$"
                        SELECT *
                        FROM pe_setting.OrderRoutingTables
                        WHERE INPUT_TIME IN ('{_From}','{_To}')"
                }
            };
                        ...
                        using (var reader = cmd.ExecuteReader())
                        {
                            while (reader.Read())
                            {
                                ...
                                for (int i = 0; i < reader.FieldCount; i++)
                                {
                                    ...
                                    //if (ignored_col.Contains(colName))
                                    //{
                                    //    continue;
                                    //}
